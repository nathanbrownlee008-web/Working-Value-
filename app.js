...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('btnLogin')?.addEventListener('click', async ()=>{
    ...
    if(error) return showMsg(error.message || 'Could not send magic link.', true);

    showMsg('Magic link sent. Check your inbox (and spam).');
  });

  document.getElementById('btnSignup')?.addEventListener('click', async ()=>{
    // Kept for backwards compatibility (button is hidden in UI)
    const email = document.getElementById('authEmail')?.value?.trim();
    if(!email) return showMsg('Enter your email.', true);
    showMsg('Sending magic link…');
    const { error } = await sendMagicLink(email);
  ...
  // Payments are not wired up yet. These buttons should NOT grant access.
  document.getElementById('btnClaimTrial')?.addEventListener('click', async ()=>{
    showMsg('Trial / payments aren\'t set up yet. (This button is disabled for now.)', true);
  });

  document.getElementById('btnClaimPromo')?.addEventListener('click', async ()=>{
    showMsg('Promo / payments aren\'t set up yet. (This button is disabled for now.)', true);
  });
...
// ======================================
// MAGIC LINK AUTH (NO PROMPT-ONLY SILENCE)
// ======================================
// The app already has an auth modal above. Some older builds also had a
// prompt-based login that caused "it closes and does nothing".
// We keep ONE clear flow here: send a magic link + show feedback.

async function sendMagicLink(email){
  const clean = (email || '').trim();
  if(!clean) return { error: { message: 'Enter an email.' } };

  // IMPORTANT: emailRedirectTo must be an allowed redirect URL in Supabase Auth settings.
...
// Optional: if some HTML still calls handleLoginClick(), make it work.
async function handleLoginClick(){
  const email = prompt('Enter your email (we\'ll send a magic link):');
  if(!email) return;
  showMsg('Sending magic link…');
  const { error } = await sendMagicLink(email);
  if(error) return showMsg(error.message || 'Could not send magic link.', true);
